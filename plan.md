...
- Create: `src/ticketflow/__init__.py`, `src/ticketflow/agent/__init__.py`, `src/ticketflow/config.py`
- Create: `tests/__init__.py`, `tests/conftest.py`
- Create: `docs/superpowers/specs/2026-06-10-ticketflow-design.md`
...
name = "ticketflow"
version = "0.1.0"
description = "Temporal.io learning project: durable support-ticket agent with human-in-the-loop approval"
...
`src/ticketflow/__init__.py` — empty file.
`src/ticketflow/agent/__init__.py` — empty file.
...
Create `Makefile` (careful: recipe lines must be indented with a TAB, not spaces):
...
The same dev server, containerized — so others can run the project with only
Docker + `uv` installed (no Temporal CLI needed):
...
services:
  temporal:
    ...
    ports:
      - "7233:7233"   # gRPC (SDK clients)
      - "8233:8233"   # Web UI
```

- [ ] **Step 6: Install dependencies and verify the test runner works**

Run: `make install`
Expected: resolves and installs `temporalio`, `fastapi`, `pytest`, etc.

Run: `make test`
Expected: `no tests ran` (exit code 5) — that's fine, there are no tests yet. (Make reports the non-zero exit as an error; that's expected until Task 2 adds tests.)

- [ ] **Step 7: Commit**

```bash
git add pyproject.toml uv.lock Makefile docker-compose.yml src tests docs
git commit -m "chore: scaffold ticketflow project with uv, temporalio, fastapi"
```
...
**Files:**
- Create: `src/ticketflow/models.py`
...
Expected: FAIL — `ModuleNotFoundError: No module named 'ticketflow.models'`

- [ ] **Step 3: Write the models**

`src/ticketflow/models.py`:
...
```bash
git add src/ticketflow/models.py tests/test_models.py
git commit -m "feat: add ticket domain models"
```

---

## Task 3: Agent protocol + MockAgent classification

**Files:**
- Create: `src/ticketflow/agent/base.py`
- Create: `src/ticketflow/agent/mock.py`
...
Expected: FAIL — `ModuleNotFoundError: No module named 'ticketflow.agent.mock'`

- [ ] **Step 3: Write protocol and minimal MockAgent**

`src/ticketflow/agent/base.py`:

```python
"""Agent interface — swap MockAgent for a real LLM-backed agent later."""
...
`src/ticketflow/agent/mock.py`:

```python
"""Mock agent with realistic randomness (seedable for tests)."""
...
```bash
git add src/ticketflow/agent tests/test_mock_agent.py
git commit -m "feat: add agent protocol and mock keyword classification"
```
